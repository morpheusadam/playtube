...
class MysqliDb
{
    ...
    public function get($tableName, $numRows = null, $columns = '*')
    {
        ...
        if (in_array('SQL_CALC_FOUND_ROWS', $this->_queryOptions) && $this->isPaginate == 1) {
            $this->_queryOptions = array_diff($this->_queryOptions, array('SQL_CALC_FOUND_ROWS'));
        }
    ...
    public function get__($tableName, $numRows = null, $columns = '*')
    {
        if (empty($columns)) {
            $columns = '*';
        }
    ...
    public function paginate ($table, $page, $fields = null) {
        ...
        if (!empty($_GET['last_page'])) {
            $res = $this->withTotalCount()->get__($table, Array ($offset, $this->pageLimit), $fields);
            $page = $this->getLastPage($page);
            $this->totalPages = $page;
            if (!empty($pt->page_number)) {
                $pt->page_number = $page;
            }
            $search = ' LIMIT ' . (int) $offset . ', ' . (int) $this->pageLimit;
            $offset = $this->pageLimit * ($page - 1);
            $replace = ' LIMIT ' . (int) $offset . ', ' . (int) $this->pageLimit;
            $qr = str_replace($search, $replace, $this->getLastQuery());

            $stmt = $this->mysqli()->prepare($qr);
            $stmt->execute();
            $this->_stmtError = $stmt->error;
            $this->_stmtErrno = $stmt->errno;
            $res = $this->_dynamicBindResults($stmt);
            ...
            if (!empty($res) && count($res) == $this->pageLimit && empty($_GET['last_page'])) {
    ...
    public function getLastPage($page)
    {
        ...
        preg_match($re, $this->getLastQuery(), $matches);

        if (!empty($matches) && !empty($matches[1])) {
            $qr = str_replace($matches[1], ' COUNT(*) AS count ', $this->getLastQuery());
            $re = '/(ORDER BY .*)/m';
            preg_match($re, $this->getLastQuery(), $matches);
            if (!empty($matches) && !empty($matches[0])) {
                $qr = str_replace($matches[0], ' ', $qr);
            }
            ...
            $stmt->execute();
            $_res = $this->_dynamicBindResults($stmt);
            if (!empty($_res) && !empty($_res[0])) {
                return ceil($_res[0]->count / $this->pageLimit);
            }
        }
        return $page;
    }
